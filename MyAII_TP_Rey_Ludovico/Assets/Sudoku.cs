...
public class Sudoku : MonoBehaviour
{
    ...
    bool RecuSolve(Matrix<int> matrixParent, int x, int y, int protectMaxDepth, List<Matrix<int>> solution)
    {
        ...
        // Verificar si la casilla ya está llena
        if (matrixParent[x, y] != Cell.EMPTY)
        {
            // Si la casilla siguiente está fuera del tablero, se llegó a una solución
            if (x == _bigSide - 1 && y == _bigSide - 1)
            {
                solution.Add(matrixParent.Clone());
                return true;
            }
            // Continuar con la siguiente casilla
            int nextX = (x + 1) % _bigSide;
            int nextY = y;
            if (nextX == 0) nextY++;
            return RecuSolve(matrixParent, nextX, nextY, protectMaxDepth, solution);
        }

        // Probar todos los valores posibles (1 a 9)
        for (int value = 1; value <= _bigSide; value++)
        {
            // Verificar si el valor se puede colocar en la casilla
            if (CanPlaceValue(matrixParent, value, x, y))
            {
                // Colocar el valor en la casilla
                ...
                bool result = RecuSolve(matrixParent, nextX, nextY, protectMaxDepth, solution);
                if (result)
                {
                    return true;
                }

                // Retroceder si el valor no conduce a una solución
